...
namespace BookLocal.API.Services
{
    public class EmployeeFinanceService : IEmployeeFinanceService
    {
        ...
        public EmployeeFinanceService(AppDbContext context, UserManager<User> userManager)
        {
            _context = context;
            _userManager = userManager;
        }

        private async Task<bool> CheckOwnerAccessAsync(int businessId, ClaimsPrincipal user)
        {
            var ownerId = user.FindFirstValue(ClaimTypes.NameIdentifier);
            return await _context.Businesses.AnyAsync(b => b.BusinessId == businessId && b.OwnerId == ownerId);
        }

        public async Task<(bool Success, IEnumerable<EmployeeDto>? Data, string? ErrorMessage)> GetEmployeesForHrAsync(int businessId, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, null, "Brak dostępu lub firma nie istnieje.");
        ...
        public async Task<(bool Success, IEnumerable<EmploymentContractDto>? Data, string? ErrorMessage)> GetContractsAsync(int businessId, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, null, "Brak dostępu.");
            ...
            return (true, contracts, null);
        }

        public async Task<(bool Success, EmploymentContractDto? Data, string? ErrorMessage)> CreateContractAsync(int businessId, EmploymentContractUpsertDto dto, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, null, "Brak dostępu.");
            ...
            if (emp == null) return (false, null, "Pracownik nie należy do tej firmy.");

            if (dto.ContractType != ContractType.Apprenticeship && dto.BaseSalary <= 0)
                return (false, null, "Wynagrodzenie brutto musi być większe niż 0 dla tego rodzaju umowy.");

            if (dto.EndDate.HasValue && dto.EndDate.Value <= dto.StartDate)
                return (false, null, "Data zakończenia musi być późniejsza niż data rozpoczęcia.");
            ...
            return (true, new EmploymentContractDto
            {
        ...
        public async Task<(bool Success, string? ErrorMessage)> ArchiveContractAsync(int businessId, int contractId, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, "Brak dostępu.");
            ...
            if (contract == null) return (false, "Umowa nie istnieje.");
            if (!contract.IsActive) return (true, "Umowa była już zarchiwizowana.");
        ...
        public async Task<(bool Success, EmploymentContractDto? Data, string? ErrorMessage)> UpdateContractAsync(int businessId, int contractId, EmploymentContractUpsertDto dto, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, null, "Brak dostępu.");
            ...
            if (contract == null) return (false, null, "Umowa nie istnieje.");
        ...
        public async Task<(bool Success, IEnumerable<EmployeePayrollDto>? Data, string? ErrorMessage)> GetPayrollsAsync(int businessId, int? month, int? year, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, null, "Brak dostępu.");
            ...
            return (true, payrolls, null);
        }

        public async Task<(bool Success, EmployeePayrollDto? Data, string? ErrorMessage)> GeneratePayrollAsync(int businessId, GeneratePayrollDto dto, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, null, "Brak dostępu.");
            ...
            if (!contracts.Any())
            {
                return (false, null, "Brak umowy obejmującej wybrany okres.");
            }
            ...
            if (employee == null) return (false, null, "Pracownik nie istnieje.");
            ...
            return (true, new EmployeePayrollDto
            {
        ...
        public async Task<(bool Success, string? ErrorMessage)> DeletePayrollAsync(int businessId, int payrollId, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, "Brak dostępu.");
            ...
            if (payroll == null) return (false, "Płaca nie istnieje.");
        ...
        public async Task<(bool Success, IEnumerable<HrMonthlySummaryDto>? Data, string? ErrorMessage)> GetMonthlySummaryAsync(int businessId, int endMonth, int endYear, int count, ClaimsPrincipal user)
        {
            if (!await CheckOwnerAccessAsync(businessId, user)) return (false, null, "Brak dostępu.");
