...
namespace BookLocal.API.Services
{
    public class SearchService : ISearchService
    {
        ...
        public async Task<(bool Success, PagedResultDto<ServiceSearchResultDto>? Data)> SearchServicesAsync(string? searchTerm, string? locationTerm, int? mainCategoryId, string? sortBy, int pageNumber, int pageSize)
        {
            ...
            var projectedQuery = query.Select(s => new ServiceSearchResultDto
            {
                ...
                MainCategoryName = s.ServiceCategory.MainCategory != null ? s.ServiceCategory.MainCategory.Name : string.Empty,
        ...
        public async Task<(bool Success, PagedResultDto<BusinessSearchResultDto>? Data)> SearchBusinessesAsync(string? searchTerm, string? locationTerm, int? mainCategoryId, string? sortBy, int pageNumber, int pageSize)
        {
            ...
            var projectedQuery = query.Select(b => new BusinessSearchResultDto
            {
                ...
                MainCategories = b.Categories.Where(sc => sc.MainCategory != null).Select(sc => sc.MainCategory.Name).Distinct().ToList(),
            ...
            projectedQuery = sortBy switch
            {
                ...
                "reviews_desc" => projectedQuery.OrderByDescending(b => b.ReviewCount),
                "newest_desc" => projectedQuery.OrderByDescending(b => b.CreatedAt),
        ...
        public async Task<(bool Success, PagedResultDto<ServiceCategorySearchResultDto>? Data)> SearchCategoryFeedAsync(string? searchTerm, string? locationTerm, int? mainCategoryId, string? sortBy, int pageNumber, int pageSize)
        {
            ...
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                ...
                query = query.Where(sc =>
                    ...
                    (sc.Business != null && sc.Business.Name.ToLower().Contains(term)) ||
            ...
            {
                var lTerm = locationTerm.ToLower();
                query = query.Where(sc => sc.Business != null && 
                                        ((sc.Business.City != null && sc.Business.City.ToLower().Contains(lTerm)) ||
        ...
        public async Task<(bool Success, IEnumerable<RebookSuggestionDto>? Data, string? ErrorMessage)> GetRebookSuggestionsAsync(ClaimsPrincipal user)
        {
            ...
            var suggestions = await _context.Reservations
                .AsNoTracking()
                .Where(r => r.CustomerId == userId && r.Status == ReservationStatus.Completed && r.ServiceVariant != null && r.ServiceVariant.Service != null && r.ServiceVariant.Service.ServiceCategory != null && r.Business != null)
                .GroupBy(r => new
                {
                ...
                .Select(g => new RebookSuggestionDto
                {
                    ...
                    EmployeeName = g.OrderByDescending(r => r.StartTime).First().Employee != null ? g.OrderByDescending(r => r.StartTime).First().Employee!.FirstName + " " + g.OrderByDescending(r => r.StartTime).First().Employee!.LastName : "Nieznany",
                    EmployeePhotoUrl = g.OrderByDescending(r => r.StartTime).First().Employee != null ? g.OrderByDescending(r => r.StartTime).First().Employee!.PhotoUrl : null,
